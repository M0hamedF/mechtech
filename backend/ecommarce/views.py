...
from .serializer import log,shop,newPass,cart,sale
...
def getUser(request,pk):
  ...
  serializer=log(users,many=False)
  return Response(serializer.data)
...
def getOrders(request):
  ...
  serializer=cart(order,many=True)
  return Response(serializer.data)

@api_view(['GET'])
def getOrder(request,pk):
  order=Order.objects.get(id=pk)
  serializer=cart(order,many=False)
  return Response(serializer.data)
...
def updateOrder(request,pk):
  order=Order.objects.get(id=pk)
  serializer=cart(order,data=request.data,many=False)
  if serializer.is_valid():
    serializer.save()
    return redirect('orders')
  return Response(serializer.data)
...
def deleteOrder(request,pk):
  ...
  order.delete()
  return HttpResponse('delete order succsesss')


@api_view(['GET'])
def getCopons(request):
  copon=Copons.objects.all()
  serializer=sale(copon,many=True)
  return Response(serializer.data)

@api_view(['GET'])
def getCopon(request,pk):
  copon=Copons.objects.get(id=pk)
  serializer=cart(copon,many=False)
  return Response(serializer.data)